{ pkgs, ... }: {
  wayland.windowManager.hyprland.settings = {
    bind = [
      ...
      "$shiftMod,K, exec, bitwarden" # Launcher
      "$mod,C, exec, quickmenu" # Quickmenu
      "$shiftMod,SPACE, exec, hyprfocus-toggle" # Toggle HyprFocus
      #"$mod,P, exec,  uwsm app -- ${pkgs.planify}/bin/io.github.alainm23.planify" # Planify
      "$mod,SPACE,exec,switch=$(hyprctl devices -j | jq -r '.keyboards[] | .active_keymap' | uniq -c | [ $(wc -l) -eq 1 ] && echo 'next' || echo '0'); for device in $(hyprctl devices -j | jq -r '.keyboards[] | .name'); do hyprctl switchxkblayout $device $switch; done; activeKeymap=$(hyprctl devices -j | jq -r '.keyboards[0] | .active_keymap'); if [ $switch == '0' ]; then resetStr='(reset)'; else resetStr=''; fi; hyprctl notify -1 1500 0 '$activeKeymap $resetStr';"
